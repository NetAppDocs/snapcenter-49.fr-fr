...
Le plug-in pour Oracle Database automatise la sauvegarde, le catalogage et le décatalogage grâce à Oracle Recovery Manager (RMAN), la vérification, le montage, le démontage, la restauration, Récupération et clonage de bases de données Oracle dans votre environnement SnapCenter.
Le plug-in pour base de données Oracle installe le plug-in SnapCenter pour UNIX afin d'effectuer toutes les opérations de protection des données.
